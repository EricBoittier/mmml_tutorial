#!/usr/bin/env bash
# Fresh PhysNet training on a filtered (outlier-removed) learning-curve split.
#
# Example:
#   cd ~/mmml_tutorial/acodcm
#   bash scripts/train_lc_filtered_fresh.sh --submit aco_n3200_r3
set -euo pipefail
...
config_path="${ROOT}/configs/learning_curve/${JOB_KEY}_clean.yaml"
ckpt_dir="${ROOT}/ckpts/learning_curve/${EPOCH_TAG}/${dataset}/n${n_train}/r${repeat}_clean"
out_dir="${ROOT}/out/eval/learning_curve/${EPOCH_TAG}/${dataset}/n${n_train}/r${repeat}_clean"
slurm_dir="${ROOT}/slurm/learning_curve/${EPOCH_TAG}"
log_file="${slurm_dir}/fresh_${JOB_KEY}_clean.out"
sbatch_file="${slurm_dir}/fresh_${JOB_KEY}_clean.sbatch"
...
mkdir -p "$slurm_dir" "$(dirname "$config_path")" "$out_dir" "$ckpt_dir"

cat > "$sbatch_file" <<EOF
#!/bin/bash
#SBATCH --job-name=lc-${dataset:0:3}-n${n_train}-r${repeat}-fresh
...
n_valid: 0
ckpt_dir: ${ckpt_dir}
tag: ${dataset}1_n${n_train}_r${repeat}_clean
seed: ${seed}
...
echo "=== [1/4] Fresh training on cleaned split (${NUM_EPOCHS} epochs) ==="
...
chmod +x "${ROOT}/scripts/train_lc_filtered_fresh.sh"

echo "Wrote ${sbatch_file}"
echo "  job=${JOB_KEY}  epochs=${NUM_EPOCHS}  outlier_mode=${OUTLIER_MODE}"
echo "  ckpt_dir=${ckpt_dir}"
echo "  eval_out=${out_dir}"

if (( DO_SUBMIT )); then
  sbatch "$sbatch_file"
else
  echo "Dry run. Submit with: bash scripts/train_lc_filtered_fresh.sh --submit ${JOB_KEY}"
fi
