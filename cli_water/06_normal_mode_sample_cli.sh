...
echo "Command: loop over out/xyz_split/*.h5 with mmml normal-mode-sample --amplitude 0.1 --max-samples 50"
...
for x in out/xyz_split/*h5; do
  mmml normal-mode-sample -i "$x" -o "$x.sampled.npz" --amplitude 0.1 --max-samples 50
done
...
echo "Output: out/xyz_split/*.sampled.npz (R, Z, N)"
